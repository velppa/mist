...
interface NewEnv extends AuthEnv {
...
/**
 * POST /new — create a document from the uploaded body and return its
 * URL as plain text. Authenticated like every write path.
 */
export async function handleNewDocument(request: Request, env: unknown): Promise<Response> {
  try {
    const newEnv = env as NewEnv;

    const auth = await authenticateNewRequest(request, newEnv);
    if (!auth.ok) {
    ...
    const stub = await getAgentByName(newEnv.DocumentAgent, id);
